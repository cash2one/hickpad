...
在 Phillip Piper 的类的基础上，新增支持 GetText 获得文本类信息(html 或者纯文本). PutText 类似，是写回去


thanks to Phillip Piper (jppx1[at]bigfoot.com)
"""
...
def getCharSet( data ):
    maping = ['gbk','gb2312','gb18030','utf-8','utf-16']
...
if __name__ == '__main__':
    ...
    get_str = GetHtml()
    get_str = get_str.replace('\xa0', '')   ## 哈哈，替换不跟网上说的一样带 u 即可: 对 的"VPSee 上个月有个 RAID0 的测试" 的确管用，主要是 VPSee 后边的空格
    # print get_str.decode('utf-8','ignore')

    ###---------- 下面代码可以转换成功了
    code = getCharSet(get_str)
    print code
    ...
    # print get_str.decode('utf-8','gb18030')
    # get_str = get_str.decode('utf-8')
    # get_str = get_str.decode('gbk')
    # get_str = get_str.decode('gb2312')
    # file('tt.txt', 'a').write(html2text.html2text(get_str))



### 这里有几个经验，确实不错 http://python.6.x6.nabble.com/CPyUG-gbk-xa0-td2743303i20.html
### 包括 type 检查对象类型  
# 这里出这个异常是你代码里有 print, print 在输出 Unicode 的时候, 需要把 Unicode encode 成你终端的默认编 
# 码, Windows 上不幸就是 gbk, 而 gbk 里没有 \xa0 这个字符. 
# 解决办法就是输出之前先把 u'\xa0' 替换成 ' ': 

# --- print u'\xa0'.replace(u'\xa0', '') 

# 或者手动 encode 成 gbk, 然后 ignore 掉不能正确 encode 的字符(是 encode, 不是 decode, 已经是 
# Unicode 了你还怎么 decode, 一个操作报异常了大家应该首先检查一下该对象的 type): 

# --- print u'\xa0'.encode('gbk', 'ignore') 